...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected Path dijkstrasShortestPath(T start, T end){  
		...
		//throw NoSuchElementException if no vertex containing start can be found
		if(vertices.get(start) == null) throw new NoSuchElementException("The start vertex " + start + " does not exist in the graph.");
		//throw NoSuchElementException if no vertex containing end can be found
		if(vertices.get(end) == null) throw new NoSuchElementException("The end vertex " + end + " does not exist in the graph.");

		Vertex startVertex = vertices.get(start); 	//starting vertex
		Vertex endVertex = vertices.get(end);     	//ending vertex

		List<T> visited = new LinkedList<T>();		//linked list which holds the visited vertices

		Path startingPath = new Path(startVertex); 	//starting path containing only the source vertex

		PriorityQueue<Path> pq = new PriorityQueue<Path>(); //priority queue called pq which holds paths

		pq.add(startingPath);						//adding startingPath to the priority queue

		while(!pq.isEmpty()) {						
			Path current = pq.remove();						//removing the lowest/start path from the priority queue
			Vertex dest = current.end;						//dest vertex = current path's end vertex
			if(dest.equals(endVertex)) {					//if dest vertex = the final vertex in the path
				pq.add(current);							//add the current path back to priority queue
				break;										//break the loop
			}

			if(!visited.contains(dest.data)) {				//if dest vertex has not already been visited
				visited.add(dest.data);						//add it to the list of visited vertices

				for(Edge edge : dest.edgesLeaving) {		//for each loop running through the vertex's edges
					if(!visited.contains(edge.target.data)) {	//if the edge's target has not already been visited
						Path next = new Path(current, edge);	//create a new Path using the extended constructor
						pq.add(next);							//add the path to the priority queue
		...
		//throw a NoSuchElementException if the priority queue is empty
		if(pq.isEmpty()) throw new NoSuchElementException("Search failed to find any path from " + startVertex.data.toString() + " to " + endVertex.data.toString());

		Path shortestPath = pq.remove();	//removing the path with the lowest cost from the priority queue and creating a new path with that data
		return shortestPath;				//return the shortestPath from the start vertex to the end vertex
	}
